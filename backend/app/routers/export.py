...
from app.services.pdf_service import generate_raktseva_pdf
...
def export_pdf(db: Session = Depends(get_db)):
    """
    Export RaktSeva Blood Bank System full SPM report as a styled A4 Landscape PDF.
    Includes: Cover page, 16-Week Gantt Chart, Milestones, System Modules, Member Leaderboard.
    """
    members      = db.query(Member).all()
    contributions= db.query(Contribution).all()
    gantt_tasks  = db.query(GanttTask).order_by(GanttTask.start_week, GanttTask.id).all()
    ...
    for m in members:
        ...
        members_data.append({
            ...
            "score": score, "hours": hours,
            "tasksCompleted": completed, "tasksInProgress": inprogress,
            "logsCount": len(m_logs),
            "percentage": round((score / total_score) * 100, 1)
        })
    members_data.sort(key=lambda x: x["score"], reverse=True)
    ...
    contributions_data = [
        {"id": c.id, "member_id": c.member_id, "title": c.title,
         "category": c.category, "level": c.level, "hours": c.hours,
         "points": c.points, "task_id": c.task_id, "verified": c.verified, "date": c.date}
        for c in contributions
    ]
    gantt_tasks_data = [
        {"id": t.id, "title": t.title, "phase": t.phase,
         "start_week": t.start_week, "end_week": t.end_week,
         "duration_weeks": t.duration_weeks, "progress_pct": t.progress_pct,
         "status": t.status, "priority": t.priority,
         "points": t.points, "assignee_id": t.assignee_id, "module_id": t.module_id}
        for t in gantt_tasks
    ]
    milestones_data = [
        {"id": ms.id, "week": ms.week, "title": ms.title,
         "status": ms.status, "completed": ms.completed}
        for ms in milestones
    ]
    modules_data = [
        {"id": mod.id, "name": mod.name, "description": mod.description,
         "status": mod.status, "completion_pct": mod.completion_pct, "lead_id": mod.lead_id}
        for mod in modules
    ]
    completed_tasks = len([t for t in gantt_tasks if t.status == "Completed"])
    ...
    filename = f"RaktSeva_SPM_Report_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.pdf"
    return StreamingResponse(
        pdf_stream,
